...
export async function generateVideoToken(formData){
  ...
  try {
    ...
    if(appointment.status!== "SCHEDULED"){
      throw new Error ("This appointment is not currrently scheduled")
    }
    const now= new Date();
    const appointmentTime =new Date(appointment.startTime);
    const timeDifference =(appointmentTime - now )/(1000*60);
    if(timeDifference>30){
      throw new Error("This call will be available 30 minutes before the scheduled time")
    }
    const appointmentEndTime = new Date(appointment.endTime);
    const expirationTime =
    Math.floor(appointmentEndTime.getTime()/1000)+60*60;

    //User name and role as connection data
